...
def collect_samples(root: Path) -> Tuple[List[Sample], List[Sample]]:
    mapping = {"no_object": 0, "object_a": 1, "object_b": 1}
    fine_grained_mapping = {"no_object": 0, "object_a": 1, "object_b": 2}
        ...
        if original_label == "object_c":
            for fp in sorted(class_dir.iterdir()):
                if fp.suffix.lower() in exts:
                    ood_samples.append(Sample(fp, original_label, -1, -1))
            continue
        if original_label not in mapping: continue
        target = mapping[original_label]
        fg_target = fine_grained_mapping[original_label]
        for fp in sorted(class_dir.iterdir()):
            if fp.suffix.lower() in exts:
                in_dist_samples.append(Sample(fp, original_label, target, fg_target))
...
def train_classifier(model, train_loader, device, epochs=10, lr=3e-4, use_spatial_loss=True, model_name="resnet18"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        logging.info(f"Classifier Epoch {epoch}/{epochs} Loss: {running_loss/max(len(train_loader),1):.4f}")
    if use_spatial_loss:
        att_loss_fn.remove()
    return model

def evaluate_classifier_accuracy(model, loader, device):
    model.eval()
...
def train_siamese(model, train_loader, device, epochs=10, lr=1e-4):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for img_a, img_b, targets in train_loader:
            ...
            optimizer.step()
            running_loss += loss.item()
        logging.info(f"Siamese Epoch {epoch}/{epochs} Loss: {running_loss/max(len(train_loader),1):.4f}")
    return model
...
def evaluate_siamese_accuracy(model, loader, prototypes, device):
    """Fallback ID classifier using closest prototype map -> unified target."""
    model.eval()
    all_preds, all_targets = [], []
    with torch.no_grad():
        for images, targets, _ in loader:
            images = images.to(device)
            embs = model(images)
            sims = [F.cosine_similarity(embs, p.unsqueeze(0)) for p in prototypes.values()]
            max_sim_idx = torch.argmax(torch.stack(sims), dim=0).cpu().numpy()
            
            # map fine_grained predicted prototypes back to binary target
            # 0 -> 0, 1 -> 1, 2 -> 1
            preds = [1 if idx > 0 else 0 for idx in max_sim_idx]
...
def compute_ood_metrics(y_true, y_score):
    if len(np.unique(y_true)) < 2:
        return {"AUROC": 0.5, "AUPR": 0.5, "FPR95": 1.0, "scores": y_score.tolist(), "labels": y_true}
    auroc = roc_auc_score(y_true, y_score)
    aupr = average_precision_score(y_true, y_score)
    fpr, tpr, _ = roc_curve(y_true, y_score)
    idx = np.where(tpr >= 0.95)[0]
    fpr95 = fpr[idx[0]] if len(idx) > 0 else 1.0
    return {"AUROC": auroc, "AUPR": aupr, "FPR95": fpr95, "scores": y_score.tolist(), "labels": y_true}
...
def evaluate_extended_ood(classifier, siamese, prototypes, in_dist_loader, ood_loader, device):
    ...
    for name, method in methods.items():
        ...
        try:
            ...
            logging.error(f"Skipping {name} due to error: {e}")
...
def evaluate_localization(classifier, dataset: SonarDataset, device, model_name: str):
    ...
    for entry in dataset.entries:
        ...
        heatmap = grad_cam.generate(tensor.unsqueeze(0).to(device), target_class=1)
        x_c, _ = compute_center_of_mass(heatmap)
        preds_left_right.append(x_c < 112)
...
def main():
    ...
    logging.info(f"Binary Classifier Standard Accuracy: {class_acc:.4f}")
    ...
    logging.info("Training Siamese Network...")
    ...
    siam_acc = evaluate_siamese_accuracy(siamese, test_loader, prototypes, device)
    logging.info(f"Siamese Model Classification Accuracy (via prototypes): {siam_acc:.4f}")
    
    logging.info("Evaluating OOD Robustness...")
